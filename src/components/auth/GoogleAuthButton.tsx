    ...
    <div className="relative w-full h-[48px] rounded-[12px] group select-none">
      ...
      <div
        className={`absolute inset-0 flex items-center justify-center gap-3 border transition-all
          ${isButtonDisabled ? 'opacity-80 bg-slate-50 border-slate-300 shadow-lg ' : 'cursor-pointer bg-[#faf8f5] border-[#e8e4db] hover:border-[#7A6F5A] shadow-lg group-hover:shadow-[0_2px_12px_rgba(122,111,90,0.1)]'}
        `}
      ...
      >
        {isLoading ? (
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" className={`transition-transform ${!isButtonDisabled ? 'group-hover:scale-110' : ''}`}>
            <defs>
              {/* Gradient for the Blue section (blends into Green) */}
              <linearGradient id="grad-blue" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="#4285F4" />
                <stop offset="100%" stopColor="#34A853" />
              </linearGradient>

              {/* Gradient for the Green section (blends into Yellow) */}
              <linearGradient id="grad-green" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="#34A853" />
                <stop offset="100%" stopColor="#FBBC05" />
              </linearGradient>

              {/* Gradient for the Yellow section (blends into Red) */}
              <linearGradient id="grad-yellow" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="#FBBC05" />
                <stop offset="100%" stopColor="#EA4335" />
              </linearGradient>

              {/* Gradient for the Red section (blends into Blue) */}
            ...
            {/* Blue Segment */}
            <path fill="url(#grad-blue)" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />

            {/* Green Segment */}
            <path fill="url(#grad-green)" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />

            {/* Yellow Segment */}
            <path fill="url(#grad-yellow)" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />

            {/* Red Segment */}
            <path fill="url(#grad-red)" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
          </svg>
        )}
