        ...
        {images.length > 1 && (
          <>
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-14 bg-white/10 hover:bg-white/20 text-white rounded-full p-3 transition-all"
            >
              <ChevronLeft size={20} />
            </button>
            <button
              onClick={() => setIdx(i => (i + 1) % images.length)}
              className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-14 bg-white/10 hover:bg-white/20 text-white rounded-full p-3 transition-all"
            >
...
export default function StayPage({ isShopMode }: { isShopMode?: boolean }) {
  ...
  return (
    <div className="min-h-screen bg-[#030A07] text-white pb-24">
      {/* Hero Header */}
          ...
          <div className="flex justify-between items-center mb-8 gap-3">
            ...
            {cart.length > 0 && (
              <button
                ...
                className="relative flex items-center gap-2 bg-emerald-500 text-slate-950 px-4 py-2 rounded-full font-black text-xs shadow-lg shadow-emerald-500/20 hover:bg-emerald-400 transition-all"
              >
                <ShoppingBag size={15} />
                <span>Order ({cart.reduce((a, c) => a + c.quantity, 0)})</span>
      ...
      {allRooms.length > 0 && allProducts.length > 0 && (
        <div className="max-w-5xl mx-auto px-5 mb-6">
          <div className="flex bg-white/5 border border-white/10 p-1 rounded-2xl max-w-md">
            <button
        ...
        <div className="max-w-5xl mx-auto px-5 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredRooms.map(room => {
              const img = room.meta.imageUrl || (room.meta.images && room.meta.images[0]);
              return (
                <div key={room.id} className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden flex flex-col justify-between">
                  ...
                  <div className="p-5 space-y-3 flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="font-black text-lg text-white">{room.title}</h3>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-white/10">
                      <div>
                        ...
                        <span className="text-lg font-black text-emerald-400">KES {Number(room.basePrice).toLocaleString()}</span>
                      </div>

                      <button
                        onClick={() => addToCart({ id: room.id, name: room.title, price: Number(room.basePrice), type: "ROOM", imageUrl: img })}
                        className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-xs rounded-xl flex items-center gap-1.5 transition-all"
                      >
                        <Plus size={14} /> Book / Reserve
                      </button>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredProducts.map(prod => (
              <div key={prod.id} className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden flex flex-col justify-between">
                ...
                <div className="p-5 space-y-3 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-black text-base text-white">{prod.name}</h3>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-white/10">
                    <div>
                      ...
                      <span className="text-lg font-black text-emerald-400">KES {Number(prod.price).toLocaleString()}</span>
                    </div>

                    <button
                      onClick={() => addToCart({ id: prod.id, name: prod.name, price: Number(prod.price), type: "PRODUCT", imageUrl: prod.imageUrl })}
                      className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs rounded-xl flex items-center gap-1.5 transition-all"
                    >
      ...
      {allRooms.length === 0 && allProducts.length === 0 && (
        <div className="max-w-md mx-auto my-12 p-8 bg-white/5 border border-white/10 rounded-3xl text-center space-y-3">
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-4 left-4 right-4 z-40 max-w-xl mx-auto">
          <div className="bg-emerald-500 text-slate-950 p-4 rounded-2xl shadow-2xl flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="bg-slate-950 text-emerald-400 h-10 w-10 rounded-xl flex items-center justify-center font-black text-sm">
                {cart.reduce((a, c) => a + c.quantity, 0)}
              </div>
              <div>
                <p className="text-xs font-black uppercase tracking-wider">Total Items Selected</p>
                <p className="text-lg font-black leading-none">KES {totalCartAmount.toLocaleString()}</p>
              </div>
            </div>

            <button
              onClick={() => { setIsCartOpen(true); setIsOrdering(true); }}
              className="bg-slate-950 hover:bg-slate-900 text-white font-black text-xs uppercase tracking-widest px-5 py-3 rounded-xl flex items-center gap-2 transition-all"
            >
        ...
        {isCartOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-[#0A1410] border border-white/10 w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 space-y-6 max-h-[90vh] overflow-y-auto text-white"
            >
              ...
              <div className="space-y-3 max-h-48 overflow-y-auto pr-1">
                {cart.map(item => (
                  <div key={item.id} className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateQuantity(item.id, -1)} className="p-1 bg-white/10 rounded-lg hover:bg-white/20">
                        <Minus size={12} />
                      </button>
                      ...
                      <button onClick={() => removeFromCart(item.id)} className="p-1 text-red-400 hover:text-red-300 ml-1">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
              ...
              <form onSubmit={handleOrderSubmit} className="space-y-3 pt-3 border-t border-white/10">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:border-emerald-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:border-emerald-500 resize-none"
                  />
                </div>
      ...
      <AnimatePresence>
        {orderSuccess && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
            <div className="bg-[#0A1410] border border-emerald-500/30 p-6 rounded-3xl max-w-md w-full text-center space-y-4">
